...
-- FIXED: Handle duplicate RegistrationNames by setting to NULL for duplicates within source data
WITH ranked_guardians AS (
    SELECT 
        ...
        g."Email",
        g."ID",
        -- Rank by RegistrationName to identify duplicates (lowest ID gets to keep the username)
        ROW_NUMBER() OVER (PARTITION BY g."RegistrationName" ORDER BY g."ID") as rn
    FROM "GuardianTable" g
    WHERE g."GlobalUID" IS NOT NULL
    AND g."Name" IS NOT NULL AND g."Name" != ''
    AND g."FirstName" IS NOT NULL AND g."FirstName" != ''
    AND g."RegistrationName" IS NOT NULL
)
...
SELECT DISTINCT ON (rg."GlobalUID")
    u."UserID" AS "Nr_UserID",
    rg."GlobalUID",
    rg."XmoodID",
    rg."Salutation",
    rg."Title",
    rg."LetterSalutation",
    rg."RegistrationX",
    -- Only the first occurrence (lowest ID) keeps the RegistrationName, others get NULL
    CASE 
        WHEN rg.rn = 1 AND NOT EXISTS (
            SELECT 1 FROM "Nr_Guardians" ng 
            WHERE ng."RegistrationName" = rg."RegistrationName"
        )
        THEN rg."RegistrationName"
        ELSE NULL
    END AS "RegistrationName",
    rg."Timestamp"
FROM ranked_guardians rg
INNER JOIN "Nr_Users" u ON (
    u."FirstName" = rg."FirstName"
    AND u."LastName" = rg."Name"
    AND u."TenantID" = rg."Tenant"
    AND (u."Email" = rg."Email" OR (u."Email" IS NULL AND rg."Email" IS NULL))
)
WHERE NOT EXISTS (
    SELECT 1 FROM "Nr_Guardians" ng WHERE ng."GlobalUID" = rg."GlobalUID"
)
ORDER BY rg."GlobalUID", rg."ID";
...
-- Note: Step 9 - Portal data migration removed - portal data is now migrated directly in Step 2
...
-- **NEW: Guardians with username conflicts (RegistrationName set to NULL due to duplicates)**
...
-- **NEW: Duplicate RegistrationNames in source data (for manual review)**
-- SELECT g."RegistrationName", COUNT(DISTINCT g."GlobalUID") as guardian_count,
--        STRING_AGG(DISTINCT g."FirstName" || ' ' || g."Name", ', ') as guardian_names
-- FROM "GuardianTable" g
